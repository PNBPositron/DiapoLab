      ...
      {collection && (
        <div className="grid grid-cols-2 gap-2">
          {files.map((file) => {
            ...
            return (
              <button key={file} onClick={() => add(newImage(src, { illustrationFormat: collection === "Transhumans" ? "png" : "svg", fit: "contain", ...(collection === "Highlights" ? { tint: editorTheme.includes("dark") ? "#ffffff" : "#0a0f1f" } : {}) }))} title={`Add ${name}`} className="group brutal-border-2 brutal-press overflow-hidden bg-surface p-1 hover:border-teal">
                <div className="grid h-24 place-items-center bg-paper/80 p-2"><img src={src} alt={`${collection} illustration: ${name}`} loading="lazy" decoding="async" width={240} height={160} className="max-h-full max-w-full object-contain" draggable={false} /></div>
